...
class EventSimulator_torch(torch.nn.Module):
    ...
    def forward(self, images, timestamps):
        ...
        if self.initial_reference_values is None:
            ...
            self.timestamps_last_event = torch.zeros_like(
    ...
    def initialized_forward(self, images, timestamps):
        T, H, W = images.shape
        reference_values_over_time = torch.zeros(
            (T - 1, H, W), device=images.device, dtype=images.dtype
        )

        event_counts = torch.zeros_like(images[0]).long()

        reference_values_over_time, event_counts = esim_cuda.forward_count_events(
            images,
            self.initial_reference_values,
        ...
        events = torch.zeros(
            (total_num_events, 4), device=cumsum.device, dtype=cumsum.dtype
        )

        events = esim_cuda.forward(
            images,
            timestamps,
            self.initial_reference_values,
            reference_values_over_time,
